...
def current_location():
    ...
    for e in h_edge_list:
        e.config(image=h_normal)
    for e in v_edge_list:
        e.config(image=v_normal)
...
def delivery_track_interface(ddid, cur_loc):
    try:
        ...
        change_map(map_matrix, cmap.find_path(map_matrix, cmap.get_destination(ddid), cur_loc))
        delivered.config(command=lambda: delivered_action(ddid))
    except TclError:
        delivery_track_interface(ddid, cur_loc)
